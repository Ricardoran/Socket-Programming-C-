...
bool readMap(char received_mapid){
	...
	ifstream inFile;
	inFile.open("./map1.txt"); // open the file stream

	// ref http://www.cplusplus.com/reference/cctype/isalpha/?kw=isalpha
	while (getline(inFile, line))
	{
		...
		if (isalpha(first_char))
		{
			...
			//map1.insert(pair<char, Gragh>(read_map_id, city));
...
int main(void)
{
	...
	while(1){
		...
		printf("\nThe Server A has received input for finding graph of map <%s>\n",received_mapid);
		printf("\n");

		// read map1.txt and check if mapid exists
		...
		if(readMap(received_id_from_aws) == true){
			...
			{
				perror("ServerA: sendto aws error\n");
				exit(1);
			}

			//printf("\nThe Server A has sent checked mapid <%c> to AWS\n", checked_map_id[0]);
			//printf("\n");
			...
			// signal to end
			printf("\nThe Server A has sent Graph to AWS\n");
			printf("\n");
			...
			printf("\nThe Server A does not have the required graph id <%c>\n", received_id_from_aws);
			printf("\n");
